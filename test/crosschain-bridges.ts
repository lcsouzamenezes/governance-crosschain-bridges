...
makeSuite('Crosschain bridge tests', setupTestEnvironment, (testEnv: TestEnv) => {
  ...
  describe.skip('Queue - ArbitrumBridgeExecutor through Ethereum Aave Governance', async function () {
    it('Execute Proposal 17 - successfully queue Arbitrum transaction - duplicate polygon actions', async () => {
      ...
      // const tx = await optimismBridgeExecutor
      //   .connect(shortExecutorSigner)
      //   .queue(targets, values, signatures, calldatas, withDelegatecalls);
      // const txReceipt = await tx.wait();
      // const parsedLog = optimismBridgeExecutor.interface.parseLog(txReceipt.logs[0]);
  ...
  describe.skip('Execute Action Sets - Aave Arbitrum Governance', async function () {
  ...
  describe.skip('ArbitrumBridgeExecutor Getters - EthereumGovernanceExecutor', async function () {
